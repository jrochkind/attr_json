module JsonAttribute
  module Type
    ...
    class Model < ::ActiveModel::Type::Value
      attr_accessor :json_hashable
      def initialize(json_hashable)
        #TODO type check, it really better be a json_hashable. maybe?
        @json_hashable = json_hashable
      end

      def type
        json_hashable.to_param.underscore.to_sym
      end
      ...
      def cast(v)
        if v.nil?
          ...
          json_hashable.new(v.to_hash)
        elsif v.respond_to?(:to_h)
          json_hashable.new(v.to_h)
        else
